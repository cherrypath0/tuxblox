...
std::optional<std::string> packageInstallSubdir(const std::string& packageName, RobloxApp /*app*/) {
    // `app` is accepted in case a package ever maps differently for Player
    // and Studio; every entry in knownPackageSubdirs() today is the same for
    // both.
