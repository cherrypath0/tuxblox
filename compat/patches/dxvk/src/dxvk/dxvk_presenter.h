...
namespace dxvk {
  ...
  private:
    ...
    // At most this many pooled fences; a swap chain never needs more than its
    // own image count, and this bounds the pool if one shrinks.
    constexpr static size_t MaxPooledFences = 16u;

    // Present fences kept across a swap chain recreate. Creating one costs a
    // kernel call on some drivers, and an application that resizes its window
    // continuously recreates the swap chain on every single frame.
