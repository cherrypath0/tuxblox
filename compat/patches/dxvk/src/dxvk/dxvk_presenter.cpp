...
namespace dxvk {
  ...
  VkResult Presenter::createSwapChain() {
    ...
    for (uint32_t i = 0; i < semaphoreCount; i++) {
      ...
      // Reuse a fence left over from the previous swap chain if there is one.
      // They are waited on and reset before the old chain is torn down, so a
      // pooled fence is always idle and unsignalled here.
      if (!m_fencePool.empty()) {
        m_semaphores[i].fence = m_fencePool.back();
  ...
  void Presenter::destroySwapchain() {
    ...
    for (const auto& sem : m_semaphores) {
      ...
      // Keep the fence for the next swap chain rather than destroying it. The
      // loop above has waited on and reset every one, so they are idle. The cap
      // bounds the pool for a chain that shrinks; anything over it is freed.
      if (sem.fence && m_fencePool.size() < MaxPooledFences)
        m_fencePool.push_back(sem.fence);
      else
  ...
  void Presenter::destroyFencePool() {
    for (VkFence fence : m_fencePool)
      m_vkd->vkDestroyFence(m_vkd->device(), fence, nullptr);
