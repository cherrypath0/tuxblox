...
// Mirrors downloader.cpp's progress callback: returning nonzero aborts the
// transfer, which is what actually unblocks curl_easy_perform() promptly when
// the user hits Cancel (otherwise the thread sits here until CURLOPT_TIMEOUT).
...
// The manifest.json schema version this installer understands. Bump this
...
Manifest parseManifest(const std::string& jsonText) {
    ...
    try {
        ...
        // Fail loudly on a future schema bump rather than silently misparsing it.
...
std::string fetchManifestJson(const std::string& url, const std::atomic<bool>* cancel) {
    ...
    // Enable the progress callback purely so Cancel can abort the transfer
    // immediately instead of waiting out CURLOPT_TIMEOUT.
