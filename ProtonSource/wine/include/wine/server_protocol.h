...
/* Retrieve the number of handles a process has open. Kept at the end of the
 * protocol: a request added anywhere else shifts the number of every request
 * below it, and the numbers are compiled into each unix library separately. */
struct get_process_handle_count_request
{
    struct request_header __header;
    obj_handle_t handle;
};
struct get_process_handle_count_reply
{
    struct reply_header __header;
    unsigned int count;
    char __pad_12[4];
...
/* Retrieve the handle values a process has open, in ascending order. Kept at
 * the end of the protocol for the same reason as the request above. */
struct get_process_handle_table_request
{
    struct request_header __header;
    obj_handle_t handle;
};
struct get_process_handle_table_reply
{
    struct reply_header __header;
    unsigned int count;
